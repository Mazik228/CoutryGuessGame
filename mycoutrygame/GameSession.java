...
public abstract class GameSession {
    ...
    protected boolean processGuess(Player player, String input) {
        ...
        guessHistory.push(countryMap.get(formattedGuess).getName());

        Country gueassedCountry = countryMap.get(formattedGuess);
        double distance = gueassedCountry.calculateDistanceInMiles(targetCountry);
        ...
        if (distance == 0) {
            ...
            System.out.printf("%s is %.2f miles away from the target country.\n", gueassedCountry.getName(), distance);
            updateClosestGuesses(gueassedCountry.getName(), distance);
            printTopClosest();
            return false; 
        }
    }
